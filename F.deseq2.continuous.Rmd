...
```{r}
cts<-as.matrix(read.csv("kidney.counts.csv",header = TRUE, row.names=1))
coldata<-read.csv("kidney.sample.info.csv",header = TRUE,row.names=1)
coldata<-coldata[,c("Age","Sex","Batch")]
head(cts)
head(coldata)
dds <- DESeqDataSetFromMatrix(countData=cts,
                              colData=coldata,
                              design=~Sex+Batch+Age)
```
...
ggplot(d, aes(x=Age, y=count)) +
  ...
  #             label.x.npc = 2, label.y.npc = 0.1,
  #             formula = formula, parse = TRUE, size = 5)
dev.off()
```
# gtex
```{r}
png(filename="gtex.lung.RSPO4.png")
...
write.csv(as.data.frame(res),
         file="Age.all.decade.results.csv")
# export only the results which pass an adjusted p value threshold
resSig <- subset(resOrdered, padj < 0.1)
write.csv(as.data.frame(resSig),
           file="Age.all.decade.resultsSig.csv")
# export normaliz[ed counts
normcounts <- counts(dds, normalize=TRUE)
write.csv(as.data.frame(normcounts),
          file="Age.all.decade.ncounts.csv")
```

## Spearman Correlation
```{r}
data <- read.csv("fgtex.sunNexposed.skin.ncounts.csv", header = TRUE, row.names=1)
tdata <- as.data.frame(t(data))
age <- read.csv("fgtex.sunNexposed.skin.sample.info.csv", header = TRUE, row.names = 1)
spearmancorrel <- NULL
spearmanpvalue <- NULL
pearsoncorrel <- NULL
pearsonpvalue <- NULL
...
write.csv(as.data.frame(findata), "fgtex.sunNexposed.skin.ncounts.CC.csv")
```
...
```{r}
plotPCA(vsd, intgroup=c("Sex","Ever.Smoked"))
?plotPCA
```
